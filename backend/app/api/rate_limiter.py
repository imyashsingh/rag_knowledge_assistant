"""
Rate limiting middleware for DocuMind backend
"""
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        self.endpoint_configs = {
            ...
            "/api/v1/auth/register": {"limit": 5, "window": 300},  # 5 requests per 5 minutes
            "/api/v1/auth/login": {"limit": 10, "window": 300},   # 10 requests per 5 minutes
            "/api/v1/auth/refresh": {"limit": 20, "window": 300}, # 20 requests per 5 minutes
            
            # Document endpoints - moderate limits
            "/api/v1/documents/upload": {"limit": 10, "window": 60},  # 10 uploads per minute
            "/api/v1/documents/": {"limit": 100, "window": 60},        # 100 requests per minute
            
            # Chat endpoints - higher limits for good UX
            "/api/v1/chat/query": {"limit": 30, "window": 60},          # 30 queries per minute
            "/api/v1/chat/stats": {"limit": 50, "window": 60},         # 50 requests per minute
            
            # Health endpoints - very high limits
            "/api/v1/health/": {"limit": 1000, "window": 60},           # 1000 requests per minute
        }
    ...
    async def _check_rate_limit(
        ...
        try:
            ...
            # Add current request
            self.redis_cache.redis_client.zadd(key, {str(current_time): current_time})
